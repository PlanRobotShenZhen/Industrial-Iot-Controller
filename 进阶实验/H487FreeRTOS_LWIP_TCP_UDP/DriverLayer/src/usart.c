...
char USART2_RXBuf[1024];

const char* const g_USART_Name[5] = {"USART1", "UART7", "USART4", "UART6", "USART2"};
const char* const g_USART_Buf[5]  = {USART1_RXBuf, UART7_RXBuf, USART4_RXBuf, UART6_RXBuf, USART2_RXBuf};

SemaphoreHandle_t xSemap[5];// ´®¿ÚÐÅºÅÁ¿
SemaphoreHandle_t xMutex[5];// ´®¿Ú»¥³âËø

void USART_xSemapxMutex_Configuration(void)
{
    for (uint32_t i = 0; i < 5; i++)
	{
		xSemap[i] = xSemaphoreCreateBinary();
...
static void RS485_Mode(USART_Module* USARTx, e_RS485_Mode Mode)
{
    if (Mode == SendMode)
    {
        if (USARTx == USART1)
        {
            ...
            #endif
            xSemaphoreTake(xMutex[USART1_ID], portMAX_DELAY);
        }
        else if (USARTx == UART7)
        {
            ...
            #endif
            xSemaphoreTake(xMutex[UART7_ID], portMAX_DELAY);
        }
        else if (USARTx == USART4)
        {
            ...
            #endif
            xSemaphoreTake(xMutex[USART4_ID], portMAX_DELAY);
        }
        else if (USARTx == UART6)
        {
            ...
            #endif
            xSemaphoreTake(xMutex[UART6_ID], portMAX_DELAY);
        }
    }
    else
    {
        if (USARTx == USART1)
        {
            ...
            GPIO_Write_Delayms(GPIOA, GPIO_PIN_6, (Mode == SendMode)?Bit_SET:Bit_RESET, 1);
            #endif
            xSemaphoreGive(xMutex[USART1_ID]);
        }
        else if (USARTx == UART7)
        {
            #if UART7_RS485
            GPIO_Write_Delayms(GPIOA, GPIO_PIN_7, (Mode == SendMode)?Bit_SET:Bit_RESET, 1);
            #endif
            xSemaphoreGive(xMutex[UART7_ID]);
        }
        else if (USARTx == USART4)
        {
            #if USART4_RS485
            GPIO_Write_Delayms(GPIOE, GPIO_PIN_9, (Mode == SendMode)?Bit_SET:Bit_RESET, 1);
            #endif
            xSemaphoreGive(xMutex[USART4_ID]);
        }
        else if (USARTx == UART6)
        {
            #if UART6_RS485
            GPIO_Write_Delayms(GPIOE, GPIO_PIN_12, (Mode == SendMode)?Bit_SET:Bit_RESET, 1);
            #endif
            xSemaphoreGive(xMutex[UART6_ID]);
        }
    }
}
...
void USART_Printf(USART_Module* USARTx, const char* Format, ...)
{
    ...
    size_t total_len = vsnprintf(NULL, 0, Format, args_copy);
    va_end(args_copy);

    if (total_len > 0)
    {
        char* SendBuf = (char*)pvPortMalloc(total_len + 1);
        if (SendBuf != NULL)
        {
            vsnprintf(SendBuf, total_len + 1, Format, args);
            USART_SendStr(USARTx, SendBuf, total_len);
            vPortFree(SendBuf);
        }
        else
        {
            USART_SendStr(USARTx, "Memory allocation failed!\r\n", strlen("Memory allocation failed!\r\n"));
        }
    }
    else
    {
        USART_SendStr(USARTx, "Invalid format string!\r\n", strlen("Invalid format string!\r\n"));
    }
...
void LOG(const char* Format, ...)
{
    ...
    if (total_len > 0)
    {
        ...
        {
            vsnprintf(SendBuf, total_len + 1, Format, args);
            USART_SendStr(USART_LOG, SendBuf, total_len);
            vPortFree(SendBuf);
        }
        else
        {
            USART_SendStr(USART_LOG, "Memory allocation failed!\r\n", strlen("Memory allocation failed!\r\n"));
        }
    }
    else
    {
        USART_SendStr(USART_LOG, "Invalid format string!\r\n", strlen("Invalid format string!\r\n"));
    }
...
{	
	...
	if ((USART_GetFlagStatus(USARTx, USART_FLAG_IDLEF) != RESET) && (USART_GetIntStatus(USARTx, USART_INT_IDLEF) != RESET))
    {
		...
		xSemaphoreGiveFromISR(xSemap[x], &xHigherPriorityTaskWoken);
